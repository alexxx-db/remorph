...
Switch requires access to a Databricks catalog, schema, and volume for:
- Creating a Delta table for state management and conversion results
- Uploading input source files to the volume

**When using existing catalog, schema, and volume:**
- **Catalog and Schema**: Requires `USE CATALOG`, `USE SCHEMA`, and `CREATE TABLE` permissions
- **Volume**: Requires `READ VOLUME` and `WRITE VOLUME` permissions

**When creating new catalog, schema, or volume:**
- Appropriate creation permissions are required (Switch will attempt to create non-existing catalog, schema, or volume during installation)

### Model Serving Access

Switch supports any LLM available through Databricks Model Serving endpoints:
...
- **Provisioned Throughput**: Optionally configure Provisioned Throughput for higher capacity workloads
...
Switch uses a three-tier configuration system:
...
| `--output-folder` | Databricks workspace path for generated outputs (must start with `/Workspace/`) | `/Workspace/path/to/notebooks` |
| `--source-tech` | Source technology/dialect to convert from (see [Source Format Support](#source-format-support) for available options) | `snowflake`, `oracle`, `python`, `airflow`, etc. |
...
|-----------|-------------|
| `catalog` | Databricks Unity Catalog for Switch state tables |
| `schema` | Schema within the catalog for storing conversion results |
| `foundation_model` | Model serving endpoint for LLM-powered conversions |
...
**File location:** `/Workspace/Users/{user}/.databricks/labs/lakebridge/switch/resources/switch_config.yml`
...
| `conversion_prompt_yaml` | Custom conversion prompt YAML file path. When specified, this custom prompt is used instead of the built-in prompt for the selected `--source-tech` | `null` | Full workspace path to YAML file |
| `output_extension` | File extension for file output | `null` | Any extension (e.g., `.yml`, `.json`) |
| `sql_output_dir` | (Experimental) Directory for SQL notebooks | `null` | Full workspace path |
| `request_params` | Additional request parameters for model serving endpoint | `null` | JSON format string (e.g., `{"max_tokens": 64000}`) |
...
Then run (note: when using a custom prompt, `--source-tech` can be any value since the custom prompt takes precedence):
```bash
databricks labs lakebridge llm-transpile \
  --input-source /local/path/to/sql \
  --output-folder /Workspace/path/to/notebooks \
...
Edit `switch_config.yml` to set `source_format: "generic"` (leave other parameters unchanged), then run:
```bash
databricks labs lakebridge llm-transpile \
...
Edit `switch_config.yml` (leave other parameters unchanged):
```yaml
...
databricks labs lakebridge llm-transpile \
  ...
  --output-folder /Workspace/path/to/workflows \
  --source-tech airflow
```
...
**Location**: You can find built-in YAML files in the `databricks/labs/switch/resources/builtin_prompts/` directory within the Switch installation. These files demonstrate the proper structure and provide conversion-specific examples that you can adapt for your custom requirements.
