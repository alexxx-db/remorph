...
```
## Configure Recon Metadata and Dashboards using Notebook

The recommended way is to use the CLI following the instructions [here](../installation#configure-reconcile).
However, if you want to configure the metadata tables and dashboards using a notebook, you can use the following code snippet.
This needs to be executed only once unless you want to change the metadata configuration.
...
recon_config = ReconcileConfig(
    ...
    metadata_config=ReconcileMetadataConfig(
        catalog="lakebridge_install_test",
        schema="reconcile",
